...
<div class="w-[90%] mx-auto pt-8 lg:w-1/2 min-h-[70vh]">
    ...
    @elseif(session('bundle'))
        <div class="bg-white shadow p-4 rounded mb-3 flex justify-between items-center">

            <!-- LEFT: ITEM INFO -->
            <div>
                <h2 class="font-semibold">{{ $bundle->name }}</h2>
                <p class="text-gray-500">₱{{ $bundle->price_per_head }} each</p>
                <p class="text-sm text-gray-600">Subtotal: ₱{{ $bundleQty * $bundle->price_per_head }}</p>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <!-- UPDATE FORM -->
                <form action="{{ route('bundle.update') }}" method="POST" class="flex items-center gap-2">
                    @csrf
                    <input 
                        type="number" 
                        name="quantity" 
                ...
                <!-- REMOVE -->
                <form action="{{ route('bundle.remove') }}" method="POST">
                    @csrf
                    <button class="mt-1">
                        <img src="{{asset('images/delete-icon.png')}}" alt="" class="max-w-[35px]">
                    </button>
        ...
        <!-- TOTAL -->
        <div class="mt-6 bg-gray-100 p-4 rounded">
            <h2 class="text-xl font-bold">Total: ₱{{ $bundleQty * $bundle->price_per_head }}</h2>
        </div>
